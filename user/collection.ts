...
class UserCollection {
	...
	static async updateOne(userId: Types.ObjectId | string, userDetails: any): Promise<HydratedDocument<User>> {
		...
		if (userDetails.followUser) {
			user.following.push(userDetails.followUser as string);
		}

		if (userDetails.unfollowUser) {
			user.following.filter((f) => f !== (userDetails.unfollowUser as string));
		}
